# -*- coding: utf-8 -*-
"""
Created on Tue Mar 17 17:16:01 2020
...
if __name__ == "__main__":
    ...
    train_dataset, train_label1, train_label2, train_label3 = dataload(train_dir)
    ...
    model1 = classification1(input_size, hidden_size1, num_layers, num_classes1)  
    model1.cuda()
    ...
    for epoch in range(num_epochs):
        ...
        print('Epoch[%d/%d], Loss1: %.4f, Loss2: %.4f, Loss3: %.4f, valence_train_acc: %.4f %%, valence_test_acc:%.4f, arousal_train_acc: %.4f %%, arousal_test_acc:%.4f %%, test_map:%.4f' \
              % (epoch+1, num_epochs, running_loss1, running_loss2, running_loss3, valence_train_acc*100.0/len(trainset), \
                 valence_test_acc*100.0/len(testset),arousal_train_acc*100.0/len(trainset), arousal_test_acc*100.0/len(testset),test_map))
    
        if test_map >= a:
            torch.save(model1,'./DMRN/DMRNvio+val+aro_vio.pkl')
            print('save violence model!')
            a = test_map
            
        if valence_test_acc*100.0/len(testset) >= b:
            torch.save(model2,'./DMRN/DMRNvio+val+aro_val.pkl')
            print('save valence model!')
            b = valence_test_acc*100.0/len(testset)
       
        if arousal_test_acc*100.0/len(testset) >= c:
            torch.save(model3,'./DMRN/DMRNvio+val+aro_aro.pkl')
            print('save arousal model!')
            c = arousal_test_acc*100.0/len(testset)
