# -*- coding: utf-8 -*-
"""
Created on Fri Mar 13 15:16:55 2020
...
def dataload(dir):
    ...
    labelname = dir.split('/')[-1] + '_valenceClass.npy'
...
def dataload2(dir):
    setname = dir.split('/')[-1] + '_valexpansion_data.npy'
    setpath = os.path.join(dir,setname)
    #setpath = dir +'_set.npy'
    labelname = dir.split('/')[-1] + '_valexpansion_valenceClass.npy'
    labelpath = os.path.join(dir,labelname)
    #labelpath = dir + '_violence.npy'
...
if __name__ == "__main__":
    ...
    for epoch in range(num_epochs):
        ...
        np.savetxt('./test/test_pred_valence.txt',labellist,fmt = '%s')
        np.savetxt('./test/test_label_valence.txt',predlist,fmt = '%s')
            '''    
        ...
        if test_acc*100.0/len(testset) >= a: #and (train_acc*100.0/len(trainset)>90) :
            torch.save(model,'./model/classificaton_valence.pkl')
            print('save model!')
